...
# Onready Variables
@onready var model = $RobotArmature
...
func perform_jump():
	...
	jumpTween()
	animation.play("RobotArmature|Robot_Jump")
	velocity.y = jump_force

func perform_flip_jump():
	AudioManager.jump_sfx.play()
	AudioManager.jump_sfx.pitch_scale = 0.8
	animation.play("RobotArmature|Robot_WalkJump", -1, 2)
	velocity.y = jump_force
	await animation.animation_finished
	can_double_jump = false
	animation.play("RobotArmature|Robot_Jump", 0.5)
...
func player_animations():
	...
	if is_on_floor():
		if is_moving(): # Checks if player is moving
			animation.play("RobotArmature|Robot_Running", 0.5)
			particle_trail.emitting = true
			footsteps.stream_paused = false
		else:
			animation.play("RobotArmature|Robot_Idle", 0.5)
